...
(function () {
    ...
    function QuickBaseClient(qdbServer) {
        ...
        this.AddRecord = function (dbid, recordArray, ignoreError) {
            var xmlQDBRequest = this.initXMLRequest();
            //cc: goes through the array and uses the first and second indices to correspond to field ID and value
    ...
    $(document).ready(function () {
        ...
        $(".Frame").ready(function () {
            ...
            $("#addStaff").click(function () {
                //alert("hey");
                var newStaffElements = $("#staffType_generalInfo");

                //get the text inputs and set the value to " " (blank) 

                var newStaff = newStaffElements.clone(true).find("input:text").val("").end();

                var blankStaff = newStaff.removeAttr("input:text");
                
                // $(".grid-form").append(blankStaff);  
                $("#generalInfo").append(blankStaff);

            })

            //use onblur to save data as the user enters
            //need a hidden field that keeps the record id returned from Quickbase
            //how to save a new line of records - need logic that iterates through the DOM Elements and saves for each new line of elements
            //
           ...
           /*$(":input").blur(function () {
                var ret = qdb.AddRecord(bjy687qes, { 35: staffType[0], 8: staffType[1], 9: staffType[2], 10: staffType[3], 11: staffType[4] });
                
                alert(ret);
            }) */
         
            $("#deleteStaff").click(function() {
                if (confirm("Are you sure you want to permanantly delete this record?"))
                    //code that deletes record associated with this button
                    alert();
